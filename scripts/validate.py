...
def validate_source() -> None:
    ...
    assert re.search(r"^## Version:\s*0\.0\.19\s*$", toc, re.MULTILINE), "TOC version must be 0.0.19"
    ...
    assert "ShirsLazyTrix-v0.0.19.zip" in readme, "README download must match the TOC version"
    workflow = (ROOT / ".github/workflows/validate.yml").read_text(encoding="utf-8")
    assert "ShirsLazyTrix-v0.0.19.zip" in workflow, "workflow package check must match the TOC version"
