...
def validate_source() -> None:
    ...
    assert re.search(r"^## Version:\s*0\.0\.20\s*$", toc, re.MULTILINE), "TOC version must be 0.0.20"
    ...
    for token in (
        ...
        "TRAINER_CANCEL_X = -39",
        "TRAINER_BUTTON_BOTTOM = 81",
        "CLASS_FRAME_HEIGHT = 596",
        "TRADE_FRAME_HEIGHT = 612",
        "TRAIN_ALL_WIDTH = 76",
        "TRAIN_ALL_HEIGHT = 18",
        "TRAIN_ALL_CENTER_X = 56",
        "TRAIN_ALL_CENTER_Y = 92",
        "TRAIN_ALL_SOCKET_X = 14",
        "TRAIN_ALL_SOCKET_Y = 74",
        "TRAIN_ALL_SOCKET_WIDTH = 90",
        "TRAIN_ALL_SOCKET_HEIGHT = 31",
        "TRAINER_MONEY_SCALE = 1",
        "TRAINER_MONEY_RIGHT = 189",
        "TRAINER_MONEY_BOTTOM = 86",
        "TRAINER_MONEY_BUTTON_WIDTH = 27",
        "TRAINER_MONEY_ELEMENT_HEIGHT = 11",
        "TRAINER_MONEY_FONT_REDUCTION = 2",
        "TRAINER_MONEY_GAP = -3",
        ...
        "validTrainerCount(number)",
        "number >= 0 and number <= 1000",
        "pcall(GetTrainerServiceInfo, index)",
        "pcall(GetTrainerServiceCost, index)",
        'serviceType == "available"',
        "playerHasSkillLine(skillLine)",
        "serviceCostsAllowed(info, talentCost, professionCost)",
        "pcall(GetTrainerServiceSkillLine, index)",
        "pcall(BuyTrainerService, index)",
        "HandleTrainerOnUpdate(elapsed)",
        "TRAINER_MAX_RETRIES = 2",
        "TRAINER_MAX_PASSES = 8",
        "TRAINER_TIMEOUT_SECONDS = 3",
        "money == lastPurchaseMoney",
        'info.serviceType == "available"',
        "currentTrainerListSignature()",
        "listSignature ~= lastPurchaseListSignature",
        "buildTrainAllSnapshot()",
        "captureTrainAllClickState()",
        'info.serviceType == "unavailable"',
        "buildTrainAllSnapshot(trainAllUnlockAllowlist, trainAllPurchased)",
        "trainAllPurchased[completed.identity] = true",
        "beginFollowUpPass()",
        "seen[info.identity]",
        "findSnapshotService(entry.identity)",
        "moneyCost ~= entry.money",
        "cp1 ~= entry.talentCost",
        "cp2 ~= entry.professionCost",
        "info.skillLine ~= entry.skillLine",
        "finalMoneyCost ~= entry.money",
        "finalInfo.identity ~= entry.identity",
        'finalInfo.serviceType ~= "available"',
        "readServiceIdentityType(index)",
        "finalIdentity ~= entry.identity",
        'finalServiceType ~= "available"',
        "compactMoneyParts()",
        ...
        'CreateTexture("ShirsLazyTrixTrainerTrainAllSocket", "BORDER")',
        ...
        'CreateTexture("ShirsLazyTrixTrainerListTrackMiddle", "BACKGROUND")',
    ...
    assert "ShirsLazyTrix-v0.0.20.zip" in readme, "README download must match the TOC version"
    workflow = (ROOT / ".github/workflows/validate.yml").read_text(encoding="utf-8")
    assert "ShirsLazyTrix-v0.0.20.zip" in workflow, "workflow package check must match the TOC version"
